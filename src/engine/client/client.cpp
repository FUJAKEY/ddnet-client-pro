...
bool CClient::InitNetworkClientImpl(NETADDR BindAddr, int Conn, char *pError, size_t ErrorSize)
{
	...
	{
		str_format(pError, ErrorSize, "Could not open the network client %s while already connected.", ClientName());
		return false;
	}
	int *pPort;
	switch(Conn)
	...
	while(!m_aNetClient[Conn].Open(BindAddr))
	{
		...
		if(RemainingAttempts == 0)
		{
			if(g_Config.m_Bindaddr[0])
				str_format(pError, ErrorSize, "Could not open the network client %s, try changing or unsetting the bindaddr '%s'.", ClientName(), g_Config.m_Bindaddr);
			else
				str_format(pError, ErrorSize, "Could not open network client %s.", ClientName());
			return false;
		}
		if(BindAddr.port != 0)
			BindAddr.port = 0;
